        ...
        while(!feof($fp)) 
        {
            ...
            if($i>=1 && !empty($buffer[0]) )
            {
                ...
                if (isset($insertData['last_update_date'])) {
                    $insertData['last_update_date'] = getTimeZoneDateTime($insertData['last_update_date'], $insertData['last_update_date']);
                }
...
else
{
    ...
    if ($fp)
    {
       ...
       while (!feof($fp)) 
       {
        ...
        if($i >= 1 && !empty($buffer[0]))
        {
          $con= isset($buffer[0])?$this->db->escape(str_replace('GB','UK',$buffer[0])):'';
		 // $res_desc=date("Y-m-d", strtotime($return_date));
