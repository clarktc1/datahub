...
class Process_finance_api extends CI_Model
{
    ...
    public function fetch_product_details($user_id =null,$order_id =null,$amz_country_code =null,$country_code=null, $token=null, $number = 1)
    {
        ...
        try
        {
            ...
            $api_response_xml = $curl_res['payload'];
            ...
            if (isset($res->Error) && !empty($res->Error)) {
                ...
                $param["apiStartDate"] = $this->apiStartDate;
                $param["apiEndDate"]   = $this->apiEndDate;
                $mergeResData = array_merge($param,$resArrayCreate);
                $responseError['data']       = json_encode($mergeResData);
                $responseError['api_date']   = $this->apiStartDate;
                $newKeyLog[] = $responseError;
                $data['status_code'] = 0;
                $data['mwsNewDataLog'] = $newKeyLog;
                return $data;
            }

            $finance_data_api_Data = array();
            $finance_data_api_Data['api_data']    = $api_response_xml;
            $finance_data_api_Data['user_id']     = $user_id;
            $finance_data_api_Data['fin_country'] = $country_code;
            $finance_data_api_Data['start_date']  = $this->apiStartDate;
    ...
    public function fetch_product_details_merge_data($finance_api_data,$user_id =null,$order_id =null,$amz_country_code =null,$country_code=null, $token=null, $number = 1)
    {
        try
        {
            $res = simplexml_load_string($finance_api_data['api_data']);
            // echo "<pre>";
            // print_r($res);
            // die();
            $payload=[];
            ...
            if (isset($res->Error) && !empty($res->Error)) {
                $responseError = array();
                $responseError['table_name'] = "Api Error finance_order_data Merge";
                $responseError['user_id']    = $user_id;
                $mergeResData = (array) $res;
                $responseError['data']       = json_encode($mergeResData);
                // $responseError['data']       = json_encode($res);
                $responseError['api_date']   = $finance_api_data["start_date"];
                $newKeyLog[] = $responseError;
            }
            ...
            if (isset($res->ListFinancialEventsResult->FinancialEvents->AdjustmentEventList->AdjustmentEvent[0]) || isset($res->ListFinancialEventsByNextTokenResult->FinancialEvents->AdjustmentEventList->AdjustmentEvent[0])) {
                ...
                $checkResponse = $this->checkMwsNewApiKey($getAdjustmentEventList, $adjustmentEventUseKey, 'finance_adjustment_event_list', $user_id, $finance_api_data["start_date"]);
                ...
                foreach ($getAdjustmentEventList as $getAdjustmentEvent) {
                    ...
                    if (isset($getAdjustmentEvent->AdjustmentItemList->AdjustmentItem)) {
                        ...
                        $checkResponse = $this->checkMwsNewApiKey($adjustmentItemData, $adjustmentEventUseKey, 'finance_adjustment_event_list', $user_id, $finance_api_data["start_date"]);
                        if ($checkResponse['status'] == 1) {
                            unset($checkResponse['status']);
                            $newKeyLog[] = $checkResponse;
                        }
            ...
            if (isset($res->ListFinancialEventsResult->FinancialEvents->ServiceFeeEventList->ServiceFeeEvent[0]) || isset($res->ListFinancialEventsByNextTokenResult->FinancialEvents->ServiceFeeEventList->ServiceFeeEvent[0])) {
                ...
                $checkResponse = $this->checkMwsNewApiKey($getServiceFeeEventListCheck, $serviceFeeEventUseKey, 'finance_service_fee_event_list', $user_id, $finance_api_data["start_date"]);
                if ($checkResponse['status'] == 1) {
                    unset($checkResponse['status']);
                    $newKeyLog[] = $checkResponse;
                }
            ...
            if (isset($res->ListFinancialEventsResult->FinancialEvents->RefundEventList->ShipmentEvent[0]) || isset($res->ListFinancialEventsByNextTokenResult->FinancialEvents->RefundEventList->ShipmentEvent[0])) {
                ...
                $checkResponse = $this->checkMwsNewApiKey($getRefundEventList, $refundEventUseKey, 'finance_refund_event_list-1', $user_id, $finance_api_data["start_date"]);
                if ($checkResponse['status'] == 1) {
                    unset($checkResponse['status']);
                    $newKeyLog[] = $checkResponse;
                }
                ...
                foreach ($getRefundEventList as $getRefundEvent) {
                    ...
                    if (isset($getRefundEvent->ShipmentItemAdjustmentList)) {
                        ...
                        $checkResponse = $this->checkMwsNewApiKey($getRefundEvent->ShipmentItemAdjustmentList, $shipmentItemUseKey, 'finance_refund_event_list-2', $user_id, $finance_api_data["start_date"],$refundEventAmazonOrderId);
                        if ($checkResponse['status'] == 1) {
                            unset($checkResponse['status']);
                            $newKeyLog[] = $checkResponse;
                        }
                    ...
                    if (isset($getRefundEvent->ShipmentItemAdjustmentList->ShipmentItem)) {
                        ...
                        $checkResponse = $this->checkMwsNewApiKey($getRefundEvent->ShipmentItemAdjustmentList->ShipmentItem, $shipmentItemUseKey, 'finance_refund_event_list-3', $user_id, $finance_api_data["start_date"],$refundEventAmazonOrderId);
                        if ($checkResponse['status'] == 1) {
                            unset($checkResponse['status']);
                            $newKeyLog[] = $checkResponse;
                        }
                ...
                if ((isset($res->ListFinancialEventsResult->FinancialEvents->RefundEventList->ShipmentEvent) && isset($res->ListFinancialEventsResult->FinancialEvents->RefundEventList->ShipmentEvent->AmazonOrderId)) || (isset($res->ListFinancialEventsByNextTokenResult->FinancialEvents->RefundEventList->ShipmentEvent) && isset($res->ListFinancialEventsByNextTokenResult->FinancialEvents->RefundEventList->ShipmentEvent->AmazonOrderId))) {
                    ...
                    $refundEventUseKey = array('AmazonOrderId','PostedDate','ShipmentItemAdjustmentList','MarketplaceName','SellerOrderId');
                    $checkResponse = $this->checkMwsNewApiKey($getRefundEventList, $refundEventUseKey, 'finance_refund_event_list-1', $user_id, $finance_api_data["start_date"],$refundEventAmazonOrderId);
                    if ($checkResponse['status'] == 1) {
                        unset($checkResponse['status']);
                        $newKeyLog[] = $checkResponse;
                    }
                    ...
                    if (isset($getRefundEventList->ShipmentItemAdjustmentList)) {
                        $shipmentItemUseKey = array('ShipmentItem');
                        $checkResponse = $this->checkMwsNewApiKey($getRefundEventList->ShipmentItemAdjustmentList, $shipmentItemUseKey, 'finance_refund_event_list-2', $user_id, $finance_api_data["start_date"],$refundEventAmazonOrderId);
                        if ($checkResponse['status'] == 1) {
                            unset($checkResponse['status']);
                            $newKeyLog[] = $checkResponse;
                        }
                    ...
                    if (isset($getRefundEventList->ShipmentItemAdjustmentList->ShipmentItem)) {
                        $shipmentItemUseKey = array('ItemFeeAdjustmentList','OrderAdjustmentItemId','QuantityShipped','ItemChargeAdjustmentList','SellerSKU','PromotionAdjustmentList','ItemTaxWithheldList');
                        $checkResponse = $this->checkMwsNewApiKey($getRefundEventList->ShipmentItemAdjustmentList->ShipmentItem, $shipmentItemUseKey, 'finance_refund_event_list-3', $user_id, $finance_api_data["start_date"],$refundEventAmazonOrderId);
                        if ($checkResponse['status'] == 1) {
                            unset($checkResponse['status']);
                            $newKeyLog[] = $checkResponse;
                        }
            ...
            if (isset($res->ListFinancialEventsResult->FinancialEvents->ShipmentEventList->ShipmentEvent[0]) || isset($res->ListFinancialEventsByNextTokenResult->FinancialEvents->ShipmentEventList->ShipmentEvent[0]))
            {
                ...
                foreach ($getShipmentEventList as $key => $ShipmentEvent)
                {
                    ...
                    $checkResponse = $this->checkMwsNewApiKey($ShipmentEvent, $serviceFeeEventUseKey, 'finance_data-1', $user_id, $finance_api_data["start_date"],$amazonOrderIdForLog);
                    if ($checkResponse['status'] == 1) {
                        unset($checkResponse['status']);
                        $newKeyLog[] = $checkResponse;
                    }
                    if (isset($ShipmentEvent->ShipmentItemList->ShipmentItem->ItemFeeList)) {
                        ...
                        $checkResponse = $this->checkMwsNewApiKey($ShipmentEvent->ShipmentItemList->ShipmentItem->ItemFeeList, $serviceFeeEventUseKey, 'finance_data-4', $user_id, $finance_api_data["start_date"],$amazonOrderIdForLog);
                        if ($checkResponse['status'] == 1) {
                            unset($checkResponse['status']);
                            $newKeyLog[] = $checkResponse;
                        }
                    ...
                    if (isset($ShipmentEvent->ShipmentItemList->ShipmentItem->ItemChargeList)) {
                        ...
                        $checkResponse = $this->checkMwsNewApiKey($ShipmentEvent->ShipmentItemList->ShipmentItem->ItemChargeList, $serviceFeeEventUseKey, 'finance_data-3', $user_id, $finance_api_data["start_date"],$amazonOrderIdForLog);
                        if ($checkResponse['status'] == 1) {
                            unset($checkResponse['status']);
                            $newKeyLog[] = $checkResponse;
                        }
                    ...
                    if (isset($ShipmentEvent->ShipmentFeeList)) {
                        ...
                        $checkResponse = $this->checkMwsNewApiKey($ShipmentEvent->ShipmentFeeList, $shipmentFeeListKey, 'finance_data-10', $user_id, $finance_api_data["start_date"],$amazonOrderIdForLog);
                        if ($checkResponse['status'] == 1) {
                            unset($checkResponse['status']);
                            $newKeyLog[] = $checkResponse;
                        }
                    ...
                    if (isset($ShipmentEvent->ShipmentItemList->ShipmentItem)) {
                        ...
                        $checkResponse = $this->checkMwsNewApiKey($shipmentItemListShipmentItem, $serviceFeeEventUseKey, 'finance_data-2', $user_id, $finance_api_data["start_date"],$amazonOrderIdForLog);
                        if ($checkResponse['status'] == 1) {
                            unset($checkResponse['status']);
                            $newKeyLog[] = $checkResponse;
                        }
                            ...
                            foreach ($shipmentItemListShipmentItem as $shipmentItemLists) {
                                ...
                                if (isset($shipmentItemLists->ItemFeeList) && !empty($shipmentItemLists->ItemFeeList)) {
                                    if (isset($shipmentItemLists->ItemFeeList->FeeComponent)) {
                                        ...
                                        $checkResponse = $this->checkMwsApiMissingTypeKey('FeeType', $shipmentItemLists->ItemFeeList->FeeComponent, $serviceFeeEventUseKey, 'finance_data-7', $user_id, $finance_api_data["start_date"],$amazonOrderIdForLog);
                                        if ($checkResponse['status'] == 1) {
                                            unset($checkResponse['status']);
                                            $newKeyLog[] = $checkResponse;
                                        }
                                ...
                                if (isset($shipmentItemLists->ItemChargeList) && !empty($shipmentItemLists->ItemChargeList)) {
                                    if (isset($shipmentItemLists->ItemChargeList->ChargeComponent)) {
                                        ...
                                        $checkResponse = $this->checkMwsApiMissingTypeKey('ChargeType', $shipmentItemLists->ItemChargeList->ChargeComponent, $serviceFeeEventUseKey, 'finance_data-6', $user_id, $finance_api_data["start_date"],$amazonOrderIdForLog);
                                        if ($checkResponse['status'] == 1) {
                                            unset($checkResponse['status']);
                                            $newKeyLog[] = $checkResponse;
                                        }
                                ...
                                if (isset($shipmentItemLists->PromotionList) && !empty($shipmentItemLists->PromotionList)) {
                                    ...
                                    $checkResponse = $this->checkMwsNewApiKey($shipmentItemLists->PromotionList, $serviceFeeEventUseKey, 'finance_data-5', $user_id, $finance_api_data["start_date"],$amazonOrderIdForLog);
                                    if ($checkResponse['status'] == 1) {
                                        unset($checkResponse['status']);
                                        $newKeyLog[] = $checkResponse;
                                    }
                                ...
                                if (isset($shipmentItemLists->ItemTaxWithheldList) && !empty($shipmentItemLists->ItemTaxWithheldList)) {
                                    ...
                                    $checkResponse = $this->checkMwsNewApiKey($shipmentItemLists->ItemTaxWithheldList, $itemTaxWithheldListKey, 'finance_data-8', $user_id, $finance_api_data["start_date"],$amazonOrderIdForLog);
                                    if ($checkResponse['status'] == 1) {
                                        unset($checkResponse['status']);
                                        $newKeyLog[] = $checkResponse;
                                    }
                                    if (isset($shipmentItemLists->ItemTaxWithheldList->TaxWithheldComponent)) {
                                        ...
                                        $checkResponse = $this->checkMwsNewApiKey($taxWithheldComponent, $taxWithheldComponentKey, 'finance_data-9', $user_id, $finance_api_data["start_date"],$amazonOrderIdForLog);
                                        if ($checkResponse['status'] == 1) {
                                            unset($checkResponse['status']);
                                            $newKeyLog[] = $checkResponse;
                                        }
    ...
    public function fetch_product_details_sb_bk($user_id =null,$order_id =null,$amz_country_code =null,$country_code=null, $token=null, $number = 1)
    {
        // echo $number;
        // echo "<br>";
        // if ($number > 9) {
