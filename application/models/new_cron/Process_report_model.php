    ...
    public function request_report($user_id,$report_type,$time_frame='30')
    {
        try
        {
            ...
            if (!empty($getUserReportDates)) {
                ...
                $getReportStartDate = $getUserReportDates["date"];
                $date_now   = date('Y-m-d');
                if (strtotime($date_now) > strtotime($getReportStartDate) ) {
                    $end_date_check  = date('Y-m-d', strtotime($daysPlusForDate, strtotime($getReportStartDate)));
                    $getEndDataNowTo = $end_date_check;
                    $startDateReport = gmdate('Y-m-d\TH:i:s\Z',strtotime($getReportStartDate));
                    $addTime         = $end_date_check." 23:59:59";
                    $endDateReport   = gmdate('Y-m-d\TH:i:s\Z',strtotime($addTime));
                    if (strtotime($end_date_check) >= strtotime($date_now) ) {
                        $getEndDataNowTo = date('Y-m-d', strtotime("-1 Day", strtotime($date_now)));
                        $end_date_check  = $getEndDataNowTo." 23:59:59";
                        // $endDateReport      = date('Y-m-d\TH:i:s\Z', strtotime("-1 Day", strtotime($end_date_check)));
                        $endDateReport      = date('Y-m-d\TH:i:s\Z', strtotime($end_date_check));
                        $param['StartDate'] = $startDateReport;
                        $param['EndDate']   = $endDateReport;
                    } else {
                        $param['StartDate'] = $startDateReport;
                        $param['EndDate']   = $endDateReport;
                    }

                    if (strtotime($getReportStartDate) != strtotime($getEndDataNowTo) && strtotime($getReportStartDate) < strtotime($getEndDataNowTo)) {
                        $curl_res = $this->create_curl_request($param);
                        if($curl_res['status_code']==0)
                        {
                            throw new Exception($curl_res['status_text']);
